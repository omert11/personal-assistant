...
`Workflow({name: "..."})` ile başlatılan hazır script'ler (built-in `code-review` dahil) `agent()` çağrılarında model override taşımaz → fable oturumunda **doğrudan launch YASAK**. 
