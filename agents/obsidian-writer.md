...
> **Append guard (kayda değerlik filtresi — ZORUNLU ilk kontrol)**: Guard yalnızca **hook kaynaklı otomatik kayıtlara** uygulanır. Caller bilinçli karar verdiyse guard ATLANIR ve her zaman yazılır: kullanıcı açıkça "not al" dedi, AskUserQuestion ile onayladı, bir skill akışından geldi (`obsidian-note`, `commit` S5, `obsidian-recall`/`obsidian-search` güncelle) veya prompt'ta açık `category` verildi (örn. `command`, `pattern` — kategori listesi adım 4 ve Tag Convention ile aynıdır). Otomatik (hook tetikli) kayıtta kriter: (1) credential/sunucu/endpoint, (2) çözülen non-trivial bug + çözümü, (3) kalıcı mimari/teknik karar. Bunlara girmeyen genel bilgi, oturum özeti veya repo/CLAUDE.md/vault'ta zaten yazılı bilgi için **hiçbir dosya yazmadan** `SKIPPED: <sebep>` döndür; şüphedeysen kaydetme — vault şişkinliği aramayı ve audit'i pahalılaştırır. Guard'dan bağımsız olarak (her çağrı tipinde) adım 4'teki mevcut-not taramasında aynı bilgi birebir zaten varsa yazma, `DUPLICATE: [[note]]` döndür — caller bu iki dönüş kodunu raporunda gösterebilsin. **Bu kriter listesi kanoniktir**: `scripts/userprompt-obsidian-hint.sh` HINT'i ve `docs/index.html` kartı buradan türetilir; kriter değişirse üçü aynı commit'te güncellenir.
